# Data Access

This repository mirrors the thesis workflow selectively rather than copying
every upstream source artifact into Git. The public-safe version keeps derived
tables, coding documentation, and thesis-facing artifacts while leaving raw
materials in an offline source archive.

## Files Used to Populate This Repo

- Offline coded article-level dataset: used to populate
  `data/processed/coded_outputs.csv` and all derived summaries.
- Offline cleaned corpus table: used to populate the public metadata table
  `data/processed/corpus_metadata.csv`.
- Offline cue-hit and validation artifacts: used to populate
  `data/diagnostics/`.
- Submitted thesis appendices and submitted draft: used to reconstruct prompts,
  codebooks, and the abstract documentation in this repo.
- Submitted draft PDF: `docs/thesis_submitted_draft.pdf`

## Not Distributed in This Public-Safe Repo

The following material is intentionally excluded from the tracked public-safe
repository:

- raw corpus export files
- full article text tables
- proprietary or licensed upstream datasets
- proprietary analytics screenshots or exported figures
- sensitive interview materials

## Access Notes

- Upstream raw materials are held offline and are not redistributed through this
  repository.
- This repository is intended to document the thesis workflow and preserve
  public-safe derived artifacts rather than serve as a redistribution bundle for
  third-party source data.
- Prompts and codebooks were reconstructed from the submitted thesis appendices,
  and the submitted draft PDF is included for reference.
